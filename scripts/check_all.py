...
Runs, in order, stopping on the first failure:
  ...
  4. scripts/reverify_existing_test.py (re-verification classifier tests)
  5. scripts/generate_readme_table.py(refresh README table + table doc)
  6. scripts/generate_status_report.py (refresh status report)
  7. scripts/sync_web_data.py        (copy root data -> web/src/data)
  8. npm run build --prefix web      (only if web/package.json exists and npm
                                      is available; otherwise SKIPPED)

This script never edits data/internships.json directly. Steps 5-7 only write
...
def main():
    steps = [
        ("1/8  Validate data", lambda: run_python("validate_data.py")),
        ("2/8  Audit data quality", lambda: run_python("audit_data_quality.py")),
        ("3/8  Auto-promotion policy tests", lambda: run_python("auto_update_policy_test.py")),
        ("4/8  Re-verification classifier tests", lambda: run_python("reverify_existing_test.py")),
        ("5/8  Generate README table", lambda: run_python("generate_readme_table.py")),
        ("6/8  Generate status report", lambda: run_python("generate_status_report.py")),
        ("7/8  Sync web data", lambda: run_python("sync_web_data.py")),
    ]
    ...
    # Step 8: web build (conditional).
    header("8/8  Build web dashboard")
    ...
    for title in completed:
        print("  [OK] %s" % title)
    print("  [%s] 8/8  Build web dashboard" %
